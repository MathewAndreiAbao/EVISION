...
function Sidebar($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!--]--> <aside${attr_class(`fixed top-0 left-0 h-full w-72 z-50 flex flex-col bg-white border-r border-border-subtle shadow-xl transition-transform duration-500 ease-pop ${stringify("-translate-x-full")} lg:translate-x-0`)}><a href="/dashboard" class="block p-8 mb-4 no-underline group"><div class="flex items-center gap-4"><div class="w-12 h-12 rounded-2xl gov-header-gradient flex items-center justify-center text-white text-2xl font-black shadow-lg shadow-gov-blue/20 group-hover:scale-105 transition-transform">E</div> <div><h1 class="text-lg font-black text-text-primary leading-none tracking-tighter">Smart E-VISION</h1> <p class="text-[10px] text-text-muted font-bold uppercase tracking-[0.2em] mt-1">System Hub</p></div></div></a> <nav class="flex-1 px-4 overflow-y-auto"><ul class="space-y-2"><!--[-->`);
    const each_array = ensure_array_like(filteredItems());
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      let item = each_array[$$index];
      const Icon2 = item.icon;
      $$renderer2.push(`<li><a${attr("href", item.href)}${attr_class(`flex items-center justify-between px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200 group ${stringify(isActive(item.href) ? "bg-gov-blue text-white shadow-lg shadow-gov-blue/10" : "text-text-secondary hover:bg-surface-muted hover:text-gov-blue")}`)}><div class="flex items-center gap-3">`);
      if (Icon2) {
        $$renderer2.push("<!--[-->");
        Icon2($$renderer2, { size: 20, strokeWidth: isActive(item.href) ? 2.5 : 2 });
        $$renderer2.push("<!--]-->");
      } else {
        $$renderer2.push("<!--[!-->");
        $$renderer2.push("<!--]-->");
    ...
    if (store_get($$store_subs ??= {}, "$profile", profile)) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="p-6 border-t border-border-subtle bg-surface-muted/30"><div class="flex items-center gap-4 mb-4"><div class="w-11 h-11 rounded-2xl bg-gov-blue/10 flex items-center justify-center text-sm font-black text-gov-blue border-2 border-white shadow-sm">${escape_html(store_get($$store_subs ??= {}, "$profile", profile).full_name?.charAt(0) || "?")}</div> <div class="flex-1 min-w-0"><p class="text-sm font-black text-text-primary truncate">${escape_html(store_get($$store_subs ??= {}, "$profile", profile).full_name)}</p> <p class="text-[10px] text-text-muted font-bold uppercase tracking-wider truncate">${escape_html(store_get($$store_subs ??= {}, "$profile", profile).role)}</p></div></div> `);
      SyncStatus($$renderer2);
      $$renderer2.push(`<!----></div>`);
    } else {
      $$renderer2.push("<!--[!-->");
    }
    $$renderer2.push(`<!--]--></aside> <nav class="lg:hidden fixed bottom-6 left-6 right-6 z-40 glass-card-static border border-white/40 shadow-2xl"><div class="flex items-center justify-around p-2"><!--[-->`);
    const each_array_1 = ensure_array_like(mobileNavItems());
    for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
      let item = each_array_1[$$index_1];
      const MobileIcon = item.icon;
      $$renderer2.push(`<a${attr("href", item.href)}${attr_class(`flex flex-col items-center gap-1 p-2 rounded-xl transition-all duration-300 ${stringify(isActive(item.href) ? "text-gov-blue bg-gov-blue/10 scale-110" : "text-text-muted hover:text-gov-blue")}`)}>`);
      if (MobileIcon) {
        $$renderer2.push("<!--[-->");
        MobileIcon($$renderer2, { size: 22, strokeWidth: isActive(item.href) ? 2.5 : 2 });
        $$renderer2.push("<!--]-->");
      } else {
        $$renderer2.push("<!--[!-->");
        $$renderer2.push("<!--]-->");
...
function _layout($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (store_get($$store_subs ??= {}, "$authLoading", authLoading)) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="min-h-screen gradient-mesh flex items-center justify-center"><div class="text-center animate-fade-in"><div class="w-16 h-16 mx-auto rounded-2xl gov-header-gradient flex items-center justify-center text-white text-3xl font-bold shadow-elevated mb-4 animate-pulse-glow">V</div> <p class="text-lg text-text-secondary font-medium">INITIALIZING...</p></div></div>`);
    } else if (store_get($$store_subs ??= {}, "$user", user)) {
      $$renderer2.push("<!--[1-->");
      $$renderer2.push(`<div class="min-h-screen bg-surface">`);
      Sidebar($$renderer2);
      $$renderer2.push(`<!----> <main class="lg:ml-72 min-h-screen"><div class="p-4 pt-16 lg:pt-8 lg:p-8 pb-24 lg:pb-8 max-w-7xl mx-auto">`);
      children($$renderer2);
