<script lang="ts">
    ...
    async function updateCharts(weeklyData: number[], schoolData: any[]) {
        ...
        barChart.data.labels = schoolData.map((s) => s.name);
        barChart.data.datasets[0].data = schoolData.map((s) => s.compliant);
        barChart.data.datasets[1].data = schoolData.map((s) => s.late);
        barChart.data.datasets[2].data = schoolData.map((s) => s.nonCompliant);
        barChart.update();
    }
